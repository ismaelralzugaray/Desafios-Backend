...
export async function updateProductsInCart(req, res) {
  try {
    ...
    res.status(200).send({ status: "Succes", payload: response, message: `No pudo agregar la siguiente cantidad de productos: ${ownerProducts.length} porque son de su propiedad` });
  } catch (error) {
    req.logger.error(error);
    res.status(200).json(error.message);
  }
