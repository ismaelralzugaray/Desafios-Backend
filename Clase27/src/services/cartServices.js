...
export async function getProductsInCart (cartId){
    try {
        let cid = cartId
        let response = await cartModel.findOne({_id: cid}).populate().lean()
        return response
    } catch (error) {
        throw new Error(error)
    }
...
export async function deleteProductInCart (pid, cid){
    try {
        ...
        if(index >= 0){
            ...
            let response = await cartModel.findOneAndUpdate({_id: cid}, {products: newArray})
            return response
        }
    }
...
export async function deleteAllProducts (cid){
    ...
    try {
        let response = await cartModel.findOneAndUpdate({_id: cid}, {products: []})
        return response
    } catch (error) {
        throw new Error(error)
    }
...
export async function updateProductsInCart (cid, newProducts){
    ...
    let response = await cartModel.findByIdAndUpdate({_id: cid}, {products: productsInCart})
    return response
