...
export async function finishPurchase (req, res){
    try {
        ...
        for (let i of purchableProducts){
            let updateStock = await productServices.updateProduct(i.product._id, i.product)
        }
        
        //ELIMINAMOS LOS PRODUCTOS DEL CARRITO
        for (let i of purchableProducts){
            let cartAfterPurchase = await cartServices.deleteProductInCart(i.product._id.valueOf(), cid)
        }
        
        //CREAMOS EL TICKET
        let response = await ticketServices.createTicket(total, req.user.email)

        res.status(200).send({status:"Succes", message:`Compra realizada con exito, los siguientes productos no pudieron ser comprados por falta de stock: ${noPurchableProducts}`})
